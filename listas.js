...
function sumarLista(listaDeNumeros) {
    return listaDeNumeros.reduce((acumulado, elementoActual) => acumulado + elementoActual, 0);
}
...
function triplicarLista(listaDeNumeros) {
    return listaDeNumeros.map((elementoActual) => elementoActual * 3);
}
...
function encontrarNumeroMayor(listaDeNumeros) {
    return listaDeNumeros.reduce((mayorHastaAhora, elementoActual) => {
        return elementoActual > mayorHastaAhora ? elementoActual : mayorHastaAhora;
    }, listaDeNumeros[0]);
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
    return listaDePalabras.reduce((masCortaHastaAhora, elementoActual) => {
        return elementoActual.length < masCortaHastaAhora.length ? elementoActual : masCortaHastaAhora;
    }, listaDePalabras[0]);
}
...
function filtrarSoloPositivos(listaDeNumeros) {
    return listaDeNumeros.filter((elementoActual) => elementoActual > 0);
}
...
function contarAprobados(listaDeNotas) {
    ...
    return listaDeNotas.filter((notaActual) => notaActual >= NOTA_APROBACION).length;
}
...
function filtrarSoloTruthy(listaDeValores) {
    return listaDeValores.filter((elementoActual) => Boolean(elementoActual));
}
...
function enumerarLista(listaDePalabras) {
    ...
    if (listaDePalabras.length === 1) {
        ...
        let todasMenosUltima = listaDePalabras.slice(0, -1).join(", ");
        let ultimaPalabra = listaDePalabras[listaDePalabras.length - 1];
        oracion = todasMenosUltima + " y " + ultimaPalabra;
    }
